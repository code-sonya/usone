...
def save_punctuality(dateList):
    users = User.objects.filter(Q(employee__empStatus='Y')).exclude(employee__empCode__isnull=True).exclude(Q(employee__empDeptName='미정')).exclude(Q(employee__empName='관리자'))\
