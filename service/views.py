...
def post_service(request, postdate):
    ...
    if request.method == "POST":
        ...
        contractList = contractList.order_by('-contractDate')
        contracts = []
...
def post_vacation(request):
    ...
    if request.method == "POST":
        ...
        if vacationCategory.categoryName == '연차':
            ...
            emp.save()
        elif vacationCategory.categoryName == '근속연차':
            emp.empSpecialLeave -= vacationDay
            emp.save()
        elif vacationCategory.categoryName == '금차':
            emp.empSpecialLeave2 -= vacationDay
            emp.save()
        elif vacationCategory.categoryName == '대체휴무':
...
def copy_service(request, serviceId):
    ...
    empName = request.user.employee.empName
    empDeptName = request.user.employee.empDeptName
    serviceId = Servicereport.objects.create(
    ...
    return redirect('service:modifyservice', serviceId.serviceId)
...
def day_report(request, day=None):
    ...
    row1 = dayreport_query(empDeptName=["설계부", "감리부", "관리부"], day=day)
